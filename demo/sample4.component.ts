...
@Component({
  ...
  <div class="row mt-5">
    <div class="col-md-12">
      <h2>Sample 4 <a href="//github.com/xuemingfeng/ngx-query/blob/master/demo/sample4.component.ts" class="btn btn-sm btn-primary" target="_blank">Source</a></h2>
      <p>Custom Toolbar</p>
      <ngx-query #ngxQuery [title]="queryTitle" (query)="search($event)" (reset)="reset($event)"
        ...
        <ng-template ngx-query-toolbar-template>
          <div class="btn-group btn-group-sm ml-1">
            <button class="btn btn-secondary" (click)="action1()">Action 1</button>
            <button class="btn btn-secondary" (click)="action2()">Action 2</button>
          </div>
        </ng-template>
      </ngx-query>
